...
Everything needed to rebuild either demo from scratch. These scripts made the
exports in `../assets/`; they live here so that rebuilding a demo after a copy
edit does not mean writing them again.
...
build/
  ...
  img/               fetch-openverse.php            (find CC0 candidates)
                     chosen.php                     (which candidate, and its alt text)
                     build-assets.php               (crop the chosen ones)
                     cand/<slot>/meta.json          (where each candidate came from)
                     credits-fieldnotes.json        (what build.php reads for alt text)
```

## What runs where

Both build scripts write to a **local WordPress at
`http://localhost/kivora-demo`** and refuse to run anywhere else. That install
exists only to be rebuilt; every run clears the previous one first.
