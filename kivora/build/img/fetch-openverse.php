<?php
/**
 * Fetch CC0 candidates from Openverse for each of a demo's image slots, and
 * build one numbered contact sheet per slot.
 *
 *     php fetch-openverse.php [--set=<demo>] [--source=<provider>] [slot]
 *
 * The slots themselves live in slots-<demo>.php; --set defaults to fieldnotes,
 * which is the demo this was first written for.
 *
...
/*
 * Which demo's slot list to search. Slot names are unique across demos, so
 * every set shares cand/ and sheets/ without colliding.
 */
...
foreach ( $args as $arg ) {
	...
	/*
	 * --source=stocksnap restricts the search to one provider. Worth having:
	 * rawpixel is a large share of Openverse's CC0 and prints a watermark
	 * across the file it serves, so a slot can come back "full" and be entirely
	 * unusable. Comma-separated for more than one.
	 */
...
$slots_file = "$base/slots-$set.php";

if ( ! is_readable( $slots_file ) ) {
	fwrite( STDERR, "No slot list for '$set' (expected " . basename( $slots_file ) . ")\n" );
	exit( 1 );
}
